  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSearch} className="flex-1 max-w-xl">
        <div className="flex items-center gap-3 px-4 py-2.5 transition-all duration-200"
          style={{
          ...
          <span className="material-symbols-outlined shrink-0 transition-colors"
            style={{ fontSize: 18, color: searchFocused ? "var(--primary)" : "var(--outline)" }}>
            search
          </span>
          <input
          ...
          />
          {search && (
            <button type="button" onClick={() => setSearch("")} style={{ color: "var(--outline)" }}>
      ...
      <div className="flex items-center gap-1 ml-4">
        ...
        {/* City */}
        <div className="hidden md:flex items-center px-3 py-2 rounded-full text-sm font-semibold transition-colors cursor-pointer"
        ...
        {/* ── Settings gear — opens dropdown with working links ── */}
        <div className="relative" ref={settingsRef}>
          ...
          {settingsOpen && (
            <div className="absolute right-0 top-full mt-2 w-56 rounded-2xl z-50 overflow-hidden py-1"
              style={{ background: "var(--surface-container-lowest)", boxShadow: "0 16px 40px rgba(25,28,30,0.13)", border: "1px solid rgba(0,0,0,0.06)" }}>
              {[
                { icon: "person",         label: "My Profile",      href: "/profile"   },
                { icon: "view_kanban",    label: "Job Tracker",     href: "/tracker"   },
                ...
                <button key={label}
                  ...
                  onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = "var(--surface-container-low)"}
                  onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = "transparent"}
                  onClick={() => { navigate(href); setSettingsOpen(false); }}>
                  <span className="material-symbols-outlined" style={{ fontSize: 18, color: "var(--outline)" }}>{icon}</span>
                  {label}
                </button>
              ))}
              <div style={{ borderTop: "1px solid var(--surface-container-low)", margin: "4px 0" }} />
              <button
                className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-semibold text-left transition-colors"
        ...
        <button onClick={() => navigate("/profile")}
          ...
          style={{ width: 36, height: 36, borderRadius: 999, boxShadow: "0 0 0 2px var(--primary), 0 2px 8px rgba(25,28,30,0.12)" }}
          onMouseEnter={e => (e.currentTarget as HTMLElement).style.boxShadow = `0 0 0 3px var(--primary), 0 4px 12px rgba(25,28,30,0.18)`}
          onMouseLeave={e => (e.currentTarget as HTMLElement).style.boxShadow = `0 0 0 2px var(--primary), 0 2px 8px rgba(25,28,30,0.12)`}>
